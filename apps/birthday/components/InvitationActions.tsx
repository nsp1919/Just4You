    ...
    <>
      ...
      <div ref={menuRef} className="fixed bottom-5 right-5 z-50 flex flex-col items-end gap-2">
        ...
        <button
          ...
          aria-label={open ? "Close invitation actions" : "Open invitation actions"}
          aria-expanded={open}
          aria-controls="invitation-actions"
          className={`grid h-12 w-12 place-items-center text-white shadow-2xl transition-transform hover:scale-105 ${square ? "rounded-none" : "rounded-full"}`}
          style={{ background: accentColor }}
        >
          {open ? <Ellipsis size={22} /> : <Share2 size={20} />}
        </button>
      </div>
