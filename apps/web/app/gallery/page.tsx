    ...
    return snap.docs
      .map((d) => {
        ...
        return {
          ...
          occasionType: (c.occasionType as string) || "birthday",
          theme: (c.theme as string) || "galaxy",
          photo: (c.photos?.[0] as string) || "",
          views: (c.views as number) || 0,
          isBlocked: !!c.isBlocked,
        };
      })
      .filter((c) => c.slug && !c.isBlocked)
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {items.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {items.map((c) => {
              const occ = OCCASIONS.find((o) => o.id === c.occasionType);
              return (
                <a
                  key={c.slug}
                ...
                >
                  <div className="h-44 w-full overflow-hidden relative">
                    {c.photo ? (
                      <img src={c.photo} alt="" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                    ) : (
                    ...
                    <div className="absolute bottom-3 left-4 right-4">
                      ...
                      <div className="text-xs text-white/70">{occ?.label ?? "Celebration"} · 👁️ {c.views.toLocaleString()}</div>
                    </div>
                  </div>
                </a>
              );
